...

	{
		...
		config = function()
			require("org-roam").setup({
				directory = "~/org/", -- Adjust this to your Org files location
			})
		end,
	},
...
-- Function to toggle link concealing
local function toggle_link_concealing()
	local line = vim.api.nvim_get_current_line()
	local col = vim.api.nvim_win_get_cursor(0)[2]
	local link_pattern = "%[%[(.-)%]%[(.-)%]%]"
	local start, finish = line:find(link_pattern)

	if start and finish and col >= start - 1 and col <= finish - 1 then
...
-- Set up autocommands to toggle concealing
vim.api.nvim_create_autocmd({ "CursorMoved", "CursorMovedI" }, {
	pattern = "*.org",
	callback = toggle_link_concealing,
})

-- Function to follow Org mode links when I press enter
local function follow_org_link()
	local status, orgmode = pcall(require, "orgmode")
	if not status then
		print("Orgmode plugin not available")
		return
	end
	...
	if not status then
		print("Error following link with orgmode: " .. tostring(err))
		print("Attempting fallback...")

		-- Fallback to built-in gf command to follow the link
		local ok, fallback_err = pcall(vim.cmd, "normal! gf")
		if not ok then
			print("Fallback failed: " .. tostring(fallback_err))
			print("Unable to follow link. Current line content:")
			print(vim.api.nvim_get_current_line())
		else
			print("Fallback succeeded")
		end
	else
		print("Link followed successfully with orgmode")
	end
end
...
vim.api.nvim_create_autocmd("FileType", {
	pattern = "org",
	callback = function()
		vim.keymap.set("n", "<CR>", follow_org_link, { buffer = true, desc = "Follow org link" })
	end,
})
